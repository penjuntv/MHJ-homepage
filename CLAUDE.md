...
- API 인증 규칙(2026-09-08): 관리자 전용 API 는 `middleware.ts` matcher 에 열거(getUser + MFA, JSON 401/403), 공개 API 는 파일 첫 줄에 `// PUBLIC_ROUTE_OK: 이유`. 둘 다 없으면 `scripts/audit-api-auth.mjs`(source-guard)가 PR 을 막는다. 요청 제한은 `lib/rate-limit.ts`.
