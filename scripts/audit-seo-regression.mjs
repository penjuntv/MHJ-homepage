 ...
 * ⚠️ `lib/blog-preflight.mjs`(관리자 폼 체크리스트)와는 **일부러 기준이 다르다** — 이쪽은 결함 기준선
 *    (내부링크 0=ORPHAN 등, 기준선이 seo-baseline.json 에 동결), 저쪽은 발행 템플릿 목표치(내부링크 2개 등).
 *    수치를 인용할 때 어느 쪽인지 밝힐 것. 여기 임계값을 목표치로 올리면 분기 회귀 이력이 끊긴다.
