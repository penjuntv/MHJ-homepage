...
export default function ContentList({ slide }: { slide: SlideConfig }) {
  ...
  return (
    <div style={{ width: '100%', height: '100%', background: bg, display: 'flex', flexDirection: 'column', padding: '5rem 5rem 4.375rem', boxSizing: 'border-box', position: 'relative', overflow: 'hidden' }}>
      ...
      {/* Title */}
      {slide.title && (
        <h2 style={{ fontFamily: titleFont, fontSize: '2rem', fontWeight: 900, fontStyle: 'italic', color: text, lineHeight: 1.15, letterSpacing: -0.5, margin: '0 0 32px', position: 'relative', zIndex: 2 }}>
          {slide.title}
        </h2>
      )}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 0, position: 'relative', zIndex: 2 }}>
        {items.map((item, i) => (
          <div key={i} style={{ borderBottom: i < items.length - 1 ? `1px solid ${text}10` : 'none', padding: '1rem 0' }}>
            <p style={{ fontFamily: bodyFont, fontSize: '0.9375rem', fontWeight: 500, lineHeight: 1.6, color: text, opacity: 0.85, margin: 0 }}>
              {item}
            </p>
          </div>
        ))}
