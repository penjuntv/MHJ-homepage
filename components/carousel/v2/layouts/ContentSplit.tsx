...
export default function ContentSplit({ slide }: { slide: SlideConfig }) {
  ...
  return (
    <div style={{ width: '100%', height: '100%', background: bg, display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ height: '55%', width: '100%', padding: '1.25rem 1.25rem 0', boxSizing: 'border-box', flexShrink: 0 }}>
      ...
      <div style={{ flex: 1, padding: '2.5rem 5rem 4.375rem', display: 'flex', flexDirection: 'column', justifyContent: 'center', position: 'relative', zIndex: 2 }}>
        ...
        {slide.title && (
          <h2 style={{ fontFamily: titleFont, fontSize: '2rem', fontWeight: 900, fontStyle: 'italic', color: text, lineHeight: 1.15, letterSpacing: -0.5, margin: '0 0 16px' }}>
            {slide.title}
          </h2>
        )}
        {slide.body && (
