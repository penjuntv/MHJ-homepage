...
export default function SummaryChecklist({ slide }: { slide: SlideConfig }) {
  ...
  return (
    <div style={{ width: '100%', height: '100%', background: bg, display: 'flex', flexDirection: 'column', padding: '80px 80px 70px', boxSizing: 'border-box', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 24, position: 'relative', zIndex: 2 }}>
        {items.map((item, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 16 }}>
            <span style={{ fontFamily: v2Tokens.fonts.body, fontSize: 18, color: '#FFFFFF', flexShrink: 0, lineHeight: 1.4 }}>
              ✓
            </span>
            <p style={{ fontFamily: bodyFont, fontSize: 16, fontWeight: 500, lineHeight: 1.5, color: '#FFFFFF', opacity: 0.9, margin: 0 }}>
              {item}
            </p>
          </div>
        ))}
      ...
      {slide.subtitle && (
        <p style={{ fontFamily: v2Tokens.fonts.bodyKr, fontSize: 13, color: accent, opacity: 0.8, marginTop: 32, lineHeight: 1.6, position: 'relative', zIndex: 2 }}>
          {slide.subtitle}
        </p>
      )}
