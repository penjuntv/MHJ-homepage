...
export default function ContentNeoBrutalism({ slide }: { slide: SlideConfig }) {
  ...
  return (
    <div style={{ width: '100%', height: '100%', background: bg, display: 'flex', flexDirection: 'column', padding: '2.5rem', boxSizing: 'border-box', position: 'relative', overflow: 'hidden', border: '8px solid #1A1A1A' }}>
      {/* Tag */}
      <div style={{ position: 'absolute', top: 20, right: 20, background: '#1A1A1A', color: '#FFFFFF', fontFamily: v2Tokens.fonts.mono, fontSize: v2Tokens.fontSize.label, fontWeight: 700, letterSpacing: v2Tokens.letterSpacing.label, lineHeight: v2Tokens.lineHeight.label, padding: '0.375rem 0.875rem', border: '2px solid #1A1A1A', boxShadow: '2px 2px 0px 0px rgba(255,255,255,1)', zIndex: 2 }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', position: 'relative', zIndex: 2, marginTop: 40 }}>
        ...
        <div style={{ background: '#FFFFFF', border: '4px solid #1A1A1A', padding: '1.5rem', boxShadow: '8px 8px 0px 0px rgba(0,0,0,1)', marginBottom: 32, transform: 'rotate(-1deg)' }}>
          {slide.title && (
            <h2 style={{ fontFamily: v2Tokens.fonts.mono, fontSize: v2Tokens.fontSize.title, fontWeight: 900, color: '#1A1A1A', lineHeight: v2Tokens.lineHeight.title, letterSpacing: v2Tokens.letterSpacing.hero, textWrap: 'balance' as const, textTransform: 'uppercase', margin: 0 }}>
              {slide.title}
            </h2>
          )}
        </div>
        ...
        {slide.body && (
          <div style={{ marginTop: 32, background: v2Tokens.palette.peach, border: '4px solid #1A1A1A', padding: '1rem', boxShadow: '4px 4px 0px 0px rgba(0,0,0,1)' }}>
            <p style={{ fontFamily: bodyFont, fontSize: v2Tokens.fontSize.body, fontWeight: 700, color: '#1A1A1A', lineHeight: v2Tokens.lineHeight.body, margin: 0 }}>
              {slide.body}
            </p>
          </div>
        )}
