  ...
  return (
    <div style={{ width: '100%', height: '100%', position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
      ...
      {(slide.title || slide.body) && (
        <div style={{ position: 'relative', zIndex: 2, textAlign: 'center', padding: `0 ${v2Tokens.safeZone.sides}`, maxWidth: 900, ...(textBgStyle ?? {}) }}>
          <p style={{ fontFamily: titleFont, fontSize: v2Tokens.fontSize.title, fontWeight: 700, fontStyle: 'italic', color: textBgStyle?.color ?? '#FFFFFF', lineHeight: v2Tokens.lineHeight.title, letterSpacing: v2Tokens.letterSpacing.hero, textWrap: 'balance' as const, textAlign: 'center', margin: 0, textShadow: textBgStyle ? 'none' : '0 2px 20px rgba(0,0,0,0.4)' }}>
            {slide.body || slide.title}
          </p>
          {slide.body && slide.title && (
            <p style={{ fontFamily: v2Tokens.fonts.body, fontSize: v2Tokens.fontSize.caption, fontWeight: 700, color: accent, marginTop: 24, lineHeight: v2Tokens.lineHeight.caption, letterSpacing: v2Tokens.letterSpacing.label, textAlign: 'center' }}>
              — {slide.title}
            </p>
          )}
        </div>
