...
export default function CoverMagazine({ slide }: { slide: SlideConfig }) {
  ...
  return (
    <div style={{ width: '100%', height: '100%', background: bg, display: 'flex', flexDirection: 'column', padding: 40, boxSizing: 'border-box', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', position: 'relative', zIndex: 2, flexShrink: 0, padding: '0 20px' }}>
        <span style={{ fontFamily: v2Tokens.fonts.body, fontSize: 9, letterSpacing: 5, textTransform: 'uppercase', color: text, opacity: 0.5 }}>
      ...
      <div style={{ position: 'relative', flex: 1, margin: '24px 20px', zIndex: 2, overflow: 'visible' }}>
        ...
        <h1 style={{ fontFamily: titleFont, fontSize: 72, fontWeight: 900, fontStyle: 'italic', color: text, lineHeight: 0.9, letterSpacing: -3, position: 'absolute', bottom: -32, left: -16, margin: 0, zIndex: 3, mixBlendMode: 'multiply' }}>
          {slide.title || 'MHJ'}
        </h1>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '0 20px', position: 'relative', zIndex: 2, flexShrink: 0, marginBottom: 30 }}>
        {slide.body && (
          <p style={{ fontFamily: bodyFont, fontSize: 12, color: text, opacity: 0.7, maxWidth: '60%', textAlign: 'right', lineHeight: 1.6, margin: 0 }}>
            {slide.body}
          </p>
        )}
      </div>
