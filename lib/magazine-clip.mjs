...
/* 빈 블록 내부로 허용되는 것 — 공백·nbsp·<br> */
const EMPTY_INNER = '(?:\\s|&nbsp;|&#160;|<br\\s*/?>)*';
/* 끝의 빈 문단: <p></p> · <p><br></p> · <p>&nbsp;</p> · <p style="…">  </p> */
const TRAILING_EMPTY_P = new RegExp(`\\s*<p\\b[^>]*>${EMPTY_INNER}</p>\\s*$`, 'i');
/* 끝의 빈 리스트: 모든 <li> 가 비어 있는 <ul>/<ol>. TipTap 에서 리스트를 만들었다가
   내용을 지우면 <ul><li><p></p></li></ul> 가 남는다(실제 사례: articles#34).
   li 가 하나라도 내용이 있으면 매칭되지 않는다. */
const TRAILING_EMPTY_LIST = new RegExp(
  `\\s*<(ul|ol)\\b[^>]*>` +
  `(?:\\s*<li\\b[^>]*>(?:${EMPTY_INNER}|<p\\b[^>]*>${EMPTY_INNER}</p>)*</li>\\s*)*` +
  `</\\1>\\s*$`,
  'i',
);

/**
 * TipTap 이 본문 끝에 남기는 빈 블록(문단·리스트)을 제거한다.
 *
 ...
 *
 * 본문 중간의 빈 블록은 의도적 여백일 수 있으므로 건드리지 않는다($ 앵커).
 *
...
export function stripTrailingEmptyBlocks(html) {
  if (!html) return '';
  // 끝에서부터 반복 적용 — 빈 문단/리스트가 여러 개 연속일 수 있고, 리스트를 지우면
  // 그 앞의 빈 문단이 새로 끝이 되기도 한다.
  let out = html;
  let prev;
  do {
    prev = out;
    out = out.replace(TRAILING_EMPTY_P, '').replace(TRAILING_EMPTY_LIST, '');
  } while (out !== prev);
  return out.trim();
}
