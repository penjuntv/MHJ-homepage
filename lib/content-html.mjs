...
/** 자식 없이 스스로 끝나는 요소. `<div/>` 의 `/` 는 HTML 이 무시한다 — 여기 없는 요소는 `/>` 여도 연다. */
const VOID_TAGS = new Set(['area', 'base', 'br', 'col', 'embed', 'hr', 'img', 'input', 'link', 'meta', 'param', 'source', 'track', 'wbr']);
const TAG_NAME = /^<(\/?)([a-zA-Z][\w-]*)/;
const ENTITY = /&(?:#\d+|#x[\da-f]+|[a-z]\w*);/gi;

/**
 * HTML 을 **최상위 자식 블록**으로 가른다 → `{ blocks: [{start, end, tag, textLen}], gapText }` 또는
 * `null`(짝이 안 맞거나 태그가 안 닫힌 입력).
 *
 * 앞으로 한 번만 훑는다(선형). 정규식으로 태그를 찾으면 안 닫힌 `<a ` 가 수천 개인 입력에서 매 위치마다 끝까지
 * 다시 훑어 제곱 시간이 된다 — 코드리뷰 실측 200KB 에 12초(ISR 재생성 때 이벤트 루프를 그만큼 막는다).
 * 따옴표 안의 `>` 는 태그 끝이 아니다. `textLen` 은 **태그 밖** 글자만 센다(공백 제외, 엔티티는 한 글자) —
 * `stripHtml` 은 속성 안 `>` 뒤를 글자로 세어 사진만 있는 문단을 '글 문단'으로 오판했다.
 * `gapText[k]` 는 블록 k 바로 앞의 최상위 틈에 글자가 있었는지.
 */
function topLevelBlocks(src) {
  ...
  let textFrom = 0;
  const flushText = (to) => {
    if (to <= textFrom) return;
    const n = src.slice(textFrom, to).replace(ENTITY, 'x').replace(/\s+/g, '').length;
    if (depth > 0 && open) open.textLen += n;
    else if (n > 0) gapText[blocks.length] = true;
  };
  while (i < src.length) {
    ...
    const m = TAG_NAME.exec(src.slice(lt, lt + 80));
    if (!m) { i = lt + 1; continue; }            // 글 속의 '<'(예: "a < b") — 태그가 아니다
    let j = lt + m[0].length;
    let quote = '';
    ...
    if (j >= src.length) return null;           // 끝나지 않은 태그 — 망가진 입력은 건드리지 않는다
    flushText(lt);
    ...
    if (m[1] === '/') {
      ...
      if (depth === 0) { open.end = end; blocks.push(open); open = null; }
    } else if (VOID_TAGS.has(tag)) {
      if (depth === 0) blocks.push({ start: lt, end, tag, textLen: 0 });
    } else {
      if (depth === 0) open = { start: lt, end: -1, tag, textLen: 0 };
      depth += 1;
    }
    i = textFrom = end;
  }
  flushText(src.length);
  return depth === 0 ? { blocks, gapText } : null;
}

/**
 * 본문 중간에 무언가(구독 CTA)를 끼울 **안전한 지점**에서 둘로 가른다 → `[앞, 뒤]` 또는 `null`.
 *
 * `</p>` 로 단순히 쪼개면 인용·목록·표·Key takeaways 박스 **안에서** 잘린다(발행 81편 중 19편이 그런 블록을 가진다) —
 ...
 * 조건을 못 채우면 `null`(호출부가 끝에 둔다). 망가진 HTML 도 `null`. 돌려주는 두 조각은 항상 `앞 + 뒤 === 원문`.
 */
export function splitForMidInsert(html, { minParagraphs = 5, minEachSide = 2, window = [0.3, 0.7] } = {}) {
  if (!html) return null;
  const src = String(html);
  const parsed = topLevelBlocks(src);
  if (!parsed || parsed.blocks.length < 2) return null;
  const { blocks, gapText } = parsed;

  const isPara = blocks.map((b) => b.tag === 'p' && b.textLen > 0);
  const parasTotal = isPara.filter(Boolean).length;
  if (parasTotal < minParagraphs) return null;

  const total = blocks.reduce((a, b) => a + b.textLen, 0);
  let before = 0;
  let parasBefore = 0;
  let best = null;
  for (let i = 0; i < blocks.length - 1; i += 1) {
    before += blocks[i].textLen;
    if (isPara[i]) parasBefore += 1;
    if (!isPara[i] || !isPara[i + 1]) continue;
    if (gapText[i + 1]) continue;                // 두 블록 사이에 최상위 글자가 끼어 있으면 경계가 아니다
