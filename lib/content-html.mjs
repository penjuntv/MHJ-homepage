...
export function addHeadingIds(html) {
  ...
  const used = new Map();
  const out = String(html).replace(/<h2\b([^>]*)>([\s\S]*?)<\/h2>/gi, (tag, attrs, inner) => {
...
export function wrapKeyTakeaways(html) {
  ...
  return String(html).replace(
    ...
    (whole, heading, _level, inner, _gap, list) =>
      TAKEAWAYS_HEADING.test(stripHtml(inner))
        ? `<aside class="blog-takeaways">${heading}${list}</aside>`
        : whole,
  );
}
