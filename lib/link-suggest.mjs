 ...
 * @param {{ now?: number, viewCountP75?: number }} [opts]
 */
export function scoreCandidate(current, candidate, opts = {}) {
  ...
  const kw = keywordsOf(current.title, current.meta_description);
...
export function suggestLinks(current, candidates, limit = 8, opts = {}) {
  ...
  return pool
    .map((c) => ({ ...c, ...scoreCandidate(current, c, { ...opts, viewCountP75: p75 }) }))
    .filter((c) => c.score > 0)
    .sort((a, b) =>
      b.score - a.score ||
