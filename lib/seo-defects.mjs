...
export const CHECKS = [
  ...
  { key: 'meta_long',    flag: 'META_LONG',    hard: false, baseline: false, label: 'meta 160자 초과', hint: '검색 결과에서 잘린다' },
...
/**
 * `flagsOf` 가 읽는 행 필드. 화면·스크립트의 select 목록이 이걸 전부 포함해야 한다 —
 * 빠뜨리면 그 검사만 조용히 통과 처리돼 수치가 달라진다(테스트가 지킨다).
 */
export const FLAG_INPUT_FIELDS = [
  'content', 'info_block_html', 'meta_description', 'og_image_url',
  'seo_title', 'summary_ko', 'faq_json', 'tags', 'image_url', 'title', 'slug',
  'updated_at', 'created_at',
];

/** 기준선 8종만 판정할 때 필요한 최소 필드 — 주간 스크립트의 select 와 짝. */
export const BASELINE_INPUT_FIELDS = [
  'content', 'info_block_html', 'meta_description', 'og_image_url',
];
...
/** 90일 — "오래됨" 의 기준(기본값). 화면이 다른 값을 주고 싶으면 flagsOf(b, { staleDays }). */
export const STALE_DAYS = 90;

/**
 * 지금 공개돼 있는가 — 주간 감사의 `.or('publish_at.is.null,publish_at.lte.now')` 와 같은 조건.
 * 화면·테스트가 각자 적으면 조용히 갈린다(2026-09-10 리뷰에서 실제로 갈렸다).
 */
export const isLive = (row, now = Date.now()) =>
  Boolean(row?.published) && (!row?.publish_at || Date.parse(row.publish_at) <= now);
...
export function flagsOf(b, opts = {}) {
  ...
  const staleMs = (opts.staleDays ?? STALE_DAYS) * 24 * 60 * 60 * 1000;
  if (!Number.isNaN(t) && (opts.now ?? Date.now()) - t > staleMs) flags.push('STALE');

  // 검색 결과에 실제로 나가는 제목 기준(W4-B: seo_title || title)
  if ((b.meta_description ?? '').length > 160) flags.push('META_LONG');
...
/**
 * **기준선 플래그만으로** 매기는 상태. 운영 지표(seo_title 없음 등)는 지금 전 편에 붙어 있어
 * 이걸 상태에 넣으면 "정상 0" 이 영구히 고정되고 화면이 신호를 잃는다.
 * @returns {'error'|'warn'|'good'}
 */
export function baselineStatusOf(flags) {
