...
- Done: `node scripts/qa/audit-contrast.mjs` — 5화면 × 2테마. 스크린샷 3화면×2테마×2뷰포트 비교
  | | 전 | 후 |
  |---|---|---|
  | 합계 | 498 | 120 |
  | CSS 클래스가 칠한 것(=코드) | 24 | **0** |
  | `style=` 속성 | 474 | 120 (매거진 표지 114 + 본문 인라인 6) |

  전·후 모두 **같은 스크립트로 다시 잰 값**이다. 작업 중에 나온 "219" 는 테마 키가 틀린 측정기가
  라이트를 두 번 재고 플레이스홀더·gradient 를 못 보던 수치라 버렸다.
- 계획에 없던 것 셋을 실측이 끌어냈다:
  ...
  ③ 첫 측정기가 테마 키를 `theme` 로 심어(실제 키는 `mhj-theme`) **다크를 잰다며 라이트를 두 번 쟀다.**
     그래서 감사 스크립트는 이제 측정 전에 `.dark` 가 실제로 걸렸는지 확인하고, 아니면 던진다.
- **남긴 것**: `/magazine` 표지 57건(라이트·다크 동일). 편집자가 고르는 `bg_color`/`accent_color` 와
  ...
  본문(`info_block_html`)의 인라인 `#9CA3AF` 6건도 코드가 아니라 데이터 → W5 정비 큐
