...
- **배포 직후 라이브 실측(2026-09-12)**: 판정표 14/15 — `library` 가 500. 원인은 검색 코드가 아니라 **Supabase 게이트웨이의
  간헐 504**(요청 후 20ms 만에 거절, 느린 쿼리 아님): 2026-09-11 16:05 부터 뉴스레터·구독자·글 목록 조회에서도 시간당
  요청의 1~6%. 검색은 조회 4~7개를 병렬로 돌려 하나만 떨어져도 전체가 500 이었다(옛 코드는 에러를 삼켜 "No results" 로
  가렸다). → `lib/postgrest-retry.mjs`(5xx·네트워크 실패만 한 번 더, 4xx 는 그대로) + 보조 조회(본문 매치·기사·매거진·스니펫)는
  실패해도 그 부분만 빼고 돌려준다(부분 결과는 `no-store` — CDN 에 굳지 않게). 글 후보 조회가 두 번 다 실패할 때만 500.
