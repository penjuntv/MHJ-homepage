...
**☑ W6-D · 검색 품질** — F-E-03(2,3) · 노력 S — 2026-09-11 완료
- 원래 문구: 1차 제목 매치 우선 2단 쿼리 + tags/meta_description 포함 + 에러/빈결과 구분 · 2차(선택) `to_tsvector` GIN + `pg_trgm`(DDL).
  Done: 쿼리 10개 관련성 판정표
- **Done — `scripts/qa/search-relevance.mjs`**: 정답을 **사람이 정한 제목 패턴**으로 두고(정답 수는 발행 글에서 자동으로 센다)
  상위 3개를 채점한다. 쿼리 10개 + 코드리뷰가 찾은 실패 경로 5개(불용어 `how to pack a lunch`·`the library`·`what is ncea`,
  짧은 토큰 `lunch in nz`, 하이픈 `k-pop`). 1위 정답 < 0.9 또는 rel@3 < 0.9 면 exit 1 — **주간 site-audit ⑭** 로 라이브를 잰다.

  | | 전(라이브) | 후 |
  |---|---|---|
  | 1위가 정답 — 처음 10개 | 7/10 | **10/10** |
  | 평균 rel@3 — 처음 10개 | 0.58 | **1.00** |
  | 1위가 정답 — 15개 | 8/15 | **15/15** |
  | 평균 rel@3 — 15개 | 0.42 | **1.00** |
  | 평균 응답(로컬 `next start` → Supabase) | — | 149ms |

  전의 실패는 전부 구조 탓이었다 — `created_at` 최신순이라 제목이 딱 맞는 글이 본문에 한 번 언급된 최신 글 뒤로 밀리고
  (`lunch` 1위가 연극 공연 글), 6편 제한에 잘려 정답이 아예 안 나오고(`reading` 0.00), 태그를 안 봐서 `kpop` 은 0건,
  두 단어는 한 구절로만 찾아 `school report` 가 1건, 문장형(`what is ncea`)은 0건.
- **무엇을 바꿨나**: 순위를 `lib/search-rank.mjs` 순수 함수로(제목 12/10/6 · 태그 6/5/4 · 카테고리 3 · 설명 2 · 본문 1 ·
  모든 단어 +10 · 제목에 구절 +8 · 태그가 구절 그대로 +6, 동점은 조회수→최신) · 태그·meta_description·category 도 찾는다 ·
  스니펫이 매치 위치 주변(전엔 본문 앞 100자) · **에러와 빈 결과를 가른다**(전엔 에러를 삼켜 "No results" 로 보였다 — 이제
  "Search is not responding" + 다시 시도) · **매거진 검색에 발행 가드**(공개 서가·홈은 거르는데 검색만 빠져 있었다 — 미발행
  0호라 샌 적은 없다) · 새 조회도 컬럼 상수(`BLOG_SEARCH_*`)로 anon grant 가드 안 · `test-search-rank` 46케이스(source-guard).
- **코드리뷰에서 고친 것(첫 커밋이 틀렸던 곳)**
  - **본문 매치가 HTML 원문을 훑었다** — 이미지 URL 의 타임스탬프가 `7` 에(47편 → 보이는 글자 17편), class/style 이
    `strong`(70 → 5)·`data`(40 → 0)에 걸려 가짜 결과가 됐다. 첫 커밋 판정표의 `year 7` 0.67 도 "본문에 7" 이 아니라 **이미지
    URL 속 7** 때문이었다. 이제 PostgREST `imatch` 로 **태그 밖 글자의 단어 앞머리**만(`(^|>)[^<]*\m토큰`).
  - **불용어가 없었다** — to·the·and·in 이 81편 전부에 걸려 "No results" 가 영영 안 뜨고 "모든 단어 +10" 이 공짜였다.
    관련 글 추천의 목록(`lib/link-suggest.mjs`)에 2자 기능어를 더해 쓰고, 불용어를 뺀 뒤 5개 상한을 센다(핵심어가 안 밀린다).
  - **짧은 토큰이 단어 속에서 점수를 받았다** — `nz` 가 태그 `nzeducation`, `in` 이 "Changing" 안에서. 2자 이하는 단어
    중간을 치지 않고, 글자↔숫자 전환은 경계로 본다(`[Y7]` 의 7).
  - **100편에서 조용히 잘릴 구조였다** — 토큰별 `.limit(100)` 에 정렬이 없어, 흔한 단어가 발행 글 전부에 걸리는 지금 101편째부터
    임의의 글이 후보에서 빠진다(월 5~9편이면 2~4달 뒤). 후보는 한 번의 조회(가벼운 칸)로, 토큰별로는 본문 매치 **id 만**,
    정렬(id 역순) + 상한 500. 5단어 검색이 283행 145KB → 후보 1회로 줄었다.
  - 하이픈 태그(`year-7` 15개)가 DB 필터에 안 걸림 → 구절 꼴(`year-7`)을 태그 겹침에 함께 넣음 · `k-pop` 은 `kpop` 도 ·
    NFD 입력 정규화 · 한글 한 글자(`책`) 유지 · 기사 날짜 형식 혼재(`2026-09-01` vs `2026.03.24`)로 동점 순서가 뒤집히던 것 ·
    매거진 동점이 최신 호 먼저가 아니게 된 것 · 스니펫용 본문 조회에 발행 가드(CLAUDE.md 3).
  - **요청 제한 60 → 120/분 + `Retry-After`** — 모바일은 글자마다, 한글 IME 는 자모마다 요청이 나가 검색어 하나에 15번 남짓,
    학교 와이파이·통신사 CGNAT 는 IP 를 나눠 쓴다. 대신 200 응답에 `s-maxage=60, stale-while-revalidate=300` — 같은 검색어는
    CDN 이 들고 있어 함수·Supabase 까지 가지 않는다(인스턴스별 제한보다 이쪽이 진짜 방어).
  - **오버레이**: "Try again" 이 성공하면 버튼째 사라져 포커스가 대화상자 밖(`<body>`)으로 떨어짐 → 입력칸으로 먼저 ·
    실패 블록 전체가 `role="alert"` 라 재시도·타이핑마다 카테고리 목록까지 통째로 다시 읽힘 → 늘 있는 `role="status"` 한 줄 ·
    상태 4개(`failed`·`searched`·…)를 하나(`view`)로 · 검색 실패 GA 이벤트 `search_failed` · 카드 호버의 배경·보더·그림자
    (§9.1 금지) · 서가 전용 곡선과 순차 지연(§9.2·§9.3) · 패딩 16/20 → 16(§7) · 뱃지 radius 999 → 4, 썸네일 12 → 8(§8.1) ·
    스피너 인디고(§6.4, 인터랙티브 아님).
- **열어 봐야 보이던 대비 결함**: 결과가 뜬 오버레이를 axe 로 재 보니 종류 알약(Journal/Article/Magazine)이 하드코딩 인디고·분홍·초록을
  9% 배경에 얹어 **양 테마 모두 미달**(라이트 초록 2.31:1 · 다크 인디고 2.48:1) — W6-A 감사는 열린 오버레이의 결과 화면을 잰 적이
  없었다. 종류는 라벨 글자가 말하므로 토큰 회색으로(§6.4 "컬러 뱃지 금지"). 같은 카드의 radius 20px 도 12px 로(CLAUDE.md 7).
- 검증: 판정표(위) · 음성 대조군 — 같은 스크립트를 라이브(옛 코드)에 돌려 exit 1 · 오버레이 결과/429/500/빈 결과 × 2테마 ×
  2뷰포트 axe 0 · 다시 시도 후 포커스 = 입력칸 · 호버 전후 배경·보더·그림자 동일 · 요청 제한 실측 120 × 200 → 429(`Retry-After: 60`).
- **배포 직후 라이브 실측(2026-09-12)**: 판정표 14/15 — `library` 가 500. 원인은 검색 코드가 아니라 **Supabase 게이트웨이
  504** — Supabase edge 로그상 504 는 전부 Vercel 함수(iad1, 미국 동부)발이고 origin 이 5.0~6.2초 걸려 게이트웨이가 끊은
  것이다. DB 는 시드니라 요청마다 태평양을 건넌다(한가한 시간에도 origin 중앙값 0.7~1.3초, p99 2~5초). Postgres 에는
  statement timeout 이 한 건도 없었다 — 쿼리가 아니라 길이 길다. 이번 배포 전부터 뉴스레터·구독자·글 목록 조회에서도
  났다(24시간 1만4천 건 중 13건, 새벽 한가한 시간대에 몰림). 검색은 조회 4~7개를 병렬로 돌려 하나만 끊겨도 전체가 500
  이었다(옛 코드는 에러를 삼켜 "No results" 로 가렸다). → `lib/postgrest-retry.mjs`(시도마다 2.5초에 끊고 5xx·네트워크·시간
  초과만 한 번 더, 4xx 는 그대로) + 보조 조회(본문 매치·기사·매거진·스니펫)는 실패해도 그 부분만 빼고 돌려준다(부분 결과는
  `no-store`). 글 후보 조회가 두 번 다 실패할 때만 500. **근본 해법은 함수 리전을 DB 옆(syd1)으로** — 별도 PR.
- **조사 처방 중 이미 끝나 있던 것**: QuickLinks 죽은 링크(W1-B 에서 상수 파생) · `articles` 발행 가드 · ILIKE 이스케이프.
- **남긴 것**: 2차 — 점수를 SQL 로(`search_blogs(tokens)` RPC: 요청 1회·스니펫까지 한 번에) 또는 `tsvector`/`pg_trgm`. **기한은
  속도가 아니라 후보 상한 500편**이다 · 어간(`kids`≠`kid`) · 발음 구별 기호 접기(`maori` 로는 `Māori` 4편이 안 나온다 — `unaccent`) ·
  한국어 형태소("아이들이" ≠ "아이들") · 태그 원소 **안** 부분 매치는 DB 로 못 걸러 순위에서만 본다 · 인스턴스별 요청 제한의 한계.
