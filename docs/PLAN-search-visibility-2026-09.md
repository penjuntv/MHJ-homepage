...
- 원래 문구: 1차 제목 매치 우선 2단 쿼리 + tags/meta_description 포함 + 에러/빈결과 구분 · 2차(선택) `to_tsvector` GIN + `pg_trgm`(DDL).
  Done: 쿼리 10개 관련성 판정표
- **Done — `scripts/qa/search-relevance.mjs`**: 쿼리 10개의 정답을 **사람이 정한 제목 패턴**으로 두고(정답 수는 발행 글에서 자동으로
  센다) 상위 3개를 채점한다. 같은 스크립트로 전·후를 쟀다.
  ...
  두 단어는 한 구절로만 찾아 `school report` 가 1건. 유일한 1.00 미만은 `year 7`(0.67) — 3위가 "Mid-**year** Report"
  (하이픈 뒤 year 가 온전한 단어로 잡히고 본문에 7) 라 규칙대로의 동작이다.
- **무엇을 바꿨나**: 순위를 `lib/search-rank.mjs` 순수 함수로(제목 12/10/6 · 태그 6/4 · 카테고리 3 · 설명 2 · 본문 1 ·
  모든 단어 +10 · 제목에 구절 +8, 동점은 조회수→최신) · 토큰별 조회라 여러 단어의 순서가 상관없다 · 태그(같은 원소)·
  meta_description·category 도 찾는다 · 스니펫이 매치 위치 주변(전엔 본문 앞 100자) · **에러와 빈 결과를 가른다**
  (전엔 에러를 삼켜 "No results" 로 보였다 — 이제 "Search is not responding" + 다시 시도) · 요청 제한 분당 60 ·
  **매거진 검색에 발행 가드**(공개 서가·홈은 거르는데 검색만 빠져 있었다 — 미발행 0호라 샌 적은 없다) · 본문은 거르기만 하고
  스니펫용은 상위 8편만 받는다 · 새 조회도 컬럼 상수(`BLOG_SEARCH_*`)로 anon grant 가드 안 · `test-search-rank` 23케이스(source-guard).
...
- **남긴 것**: 2차 `tsvector`/`pg_trgm`(DDL — 81편에선 평균 162ms 로 충분, 수백 편이 되면) · 한국어 형태소("아이들이" ≠ "아이들") ·
  태그 원소 **안** 부분 매치는 DB 필터로 못 걸러 순위에서만 본다.
